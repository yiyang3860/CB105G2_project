...
# 主程式進入點
def imgproducer(bootstrapserver, topicname, fname, maxinflightrequestsperconnection=1):
    # 步驟1. 設定要連線到Kafka集群的相關設定
    props = {
        # Kafka集群在那裡?
        'bootstrap.servers': bootstrapserver,  # <-- 置換成要連接的Kafka集群
    ...
    try:
        ...
        for i in range(0, msgCount):
            ...
            producer.produce(topicName, key=fname[fname.rfind("\\"):-4], value=i)
